...
namespace utility
{
    namespace costream
    {
        ...
        public:
            ...
            KERBAL_CONSTEXPR Color_t(int ID = -1): ID(ID) {}
        ...
        const
          ...
          GREY(8), LIGHT_BLUE(9), LIGHT_GREEN(10), LIGHT_LIGHT_GREEN(11), LIGHT_RED(12),
        ...
        BLACK(0), RED(1), GREEN(2), YELLOW(3), BLUE(4), PURPLE(5), LIGHT_BLUE(104), WHITE(7),
          ...
          GREY(107), LIGHT_LIGHT_GREEN(106), BRIGHT_WHITE(107);
#endif
        ...
        public:
            ...
            const costream &operator<<(const Type &src) const
            {
                ...
                WORD   color =
                  (this->background == INIT ? bakup.init_background : this->background.ID) * 16 +
                  (this->foreground == INIT ? bakup.init_foreground : this->foreground.ID);
...
#ifdef _WIN32
        template <std::ostream &bind_ostream>
        const typename costream<bind_ostream>::Init_bakup
          costream<bind_ostream>::bakup(STD_OUTPUT_HANDLE);

        template <>
        const typename costream<std::cerr>::Init_bakup costream<std::cerr>::bakup(STD_ERROR_HANDLE);
#endif  // WIN32

        //			costream<std::cerr> ccerr(LIGHT_RED, INIT);

        const Color_t color_test_arr[] = { BLACK,        RED,          GREEN,
                                           YELLOW,       BLUE,         PURPLE,
                                           LIGHT_BLUE,   WHITE,        LIGHT_RED,
                                           LIGHT_GREEN,  LIGHT_YELLOW, LAKE_BLUE,
                                           LIGHT_PURPLE, GREY,         LIGHT_LIGHT_GREEN,
                                           BRIGHT_WHITE };
